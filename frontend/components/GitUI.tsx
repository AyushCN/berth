...
export function BranchPicker({
  ...
  const handleCheckout = async (branch: string) => {
    ...
    try {
      await api.git.checkout(envId, branch);
      toast.success(`Checked out ${branch}`);
      setIsOpen(false);
      onBranchChanged();
    } catch (err: any) {
      if (err.message && err.message.includes("Working tree is dirty")) {
         const force = confirm("Working tree is dirty. Do you want to force checkout and discard your changes?");
         if (force) {
           try {
             await api.git.checkout(envId, branch, true);
             toast.success(`Checked out ${branch} (Forced)`);
             setIsOpen(false);
             onBranchChanged();
           } catch (forceErr: any) {
             toast.error(forceErr.message || "Checkout failed");
           }
         }
      } else {
  ...
  const handleCreateBranch = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      await api.git.createBranch(envId, newBranchName.trim());
...
export function GitStatusPanel({ envId }: { envId: string }) {
  ...
  const { data: status, mutate, error } = useSWR(`git-status-${envId}`, () => api.git.status(envId), { refreshInterval: 10000 });
    ...
    <div className="bg-[#1a1c23] border border-outline-variant rounded-xl p-4 flex flex-col gap-3">
      ...
      {status.dirty && (
        <button
          ...
          className="w-full py-2 rounded-lg text-sm font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/20 transition-colors flex items-center justify-center gap-2 mt-2"
        >
